    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200">
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-900">Ad Accounts Overview</h2>
          <div className="flex items-center space-x-2">
            <div className="relative">
              <select
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value as AdAccount['status'] | 'All')}
                className="appearance-none bg-gray-50 border border-gray-200 text-sm rounded-lg px-3 py-2 pr-8 focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredAccounts.map((account) => (
                <tr key={account.id} className="text-sm hover:bg-gray-50">
                  <td className="py-4">
                    <div className="font-medium text-gray-900">{account.name}</div>
                  </td>
                  <td className="py-4 text-gray-500">{account.id}</td>
                  <td className="py-4 text-gray-500">{account.lastPayment}</td>
                  <td className="py-4">
